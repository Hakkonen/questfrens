...
if (process.argv.length === 3) {
    ...
    console.error("next-image-export-optimizer: Breaking change: Please provide the path to the next.config.js file as an argument with the name --nextConfigPath.");
    // Reset the color
...
let nextConfigPath = nextConfigPathIndex !== -1
    ? process.argv[nextConfigPathIndex + 1]
    : undefined;
let exportFolderPathCommandLine = exportFolderPathIndex !== -1
...
const nextImageExportOptimizer = async function () {
    ...
    const { remoteImageFilenames, remoteImageURLs } = await getRemoteImageURLs(nextConfigFolder, folderPathForRemoteImages);
    try {
        ...
        else if (newPath?.nextImageExportOptimizer_generateAndUseBlurImages !==
            undefined &&
        ...
        if (nextjsConfig.transpilePackages === undefined || // transpilePackages is not set
            ...
            console.warn("\x1b[41m", `Changed in 1.2.0: You have not set transpilePackages: ["next-image-export-optimizer"] in your next.config.js. This may cause problems with next-image-export-optimizer. Please add this line to your next.config.js.`, "\x1b[0m");
        }
    }
    catch (e) {
    ...
    if (!fs.existsSync(path.join(nextConfigFolder, "public"))) {
        console.warn("\x1b[41m", `Could not find a public folder in this directory. Make sure you run the command in the main directory of your project.`, "\x1b[0m");
    }
    ...
    if (remoteImageURLs.length > 0) {
        try {
            if (!fs.existsSync(folderNameForRemoteImages)) {
                ...
                console.log(`Create remote image output folder: ${folderNameForRemoteImages}`);
            }
            else {
    ...
    if (remoteImageURLs.length > 0)
        console.log(`Downloading ${remoteImageURLs.length} remote image${remoteImageURLs.length > 1 ? "s" : ""}...`);
    await downloadImagesInBatches(remoteImageURLs, remoteImageFilenames, folderPathForRemoteImages, Math.min(remoteImageURLs.length, 20));
    ...
    const isImageFolderSubdirectoryOfPublicFolder = imageFolderPath.includes("public");
    const allFilesInImageFolderAndSubdirectories = isImageFolderSubdirectoryOfPublicFolder
        ? getAllFilesAsObject(imageFolderPath, imageFolderPath, exportFolderName)
        : [];
    const allFilesInStaticImageFolder = getAllFilesAsObject(staticImageFolderPath, staticImageFolderPath, exportFolderName);
    ...
    const allImagesInImageFolder = allFilesInImageFolderAndSubdirectories.filter((fileObject) => {
    ...
    console.log(`Found ${allImagesInImageFolder.length - remoteImageURLs.length} supported images in ${imageFolderPath}, static folder and subdirectories and ${remoteImageURLs.length} remote image${remoteImageURLs.length > 1 ? "s" : ""}.`);
    ...
    if (allImagesInImageFolder.length > 0) {
        ...
        console.log(`Start optimization of ${allImagesInImageFolder.length} images with ${widths.length} sizes resulting in ${allImagesInImageFolder.length * widths.length} optimized images...`);
    ...
    for (let index = 0; index < allImagesInImageFolder.length; index++) {
        ...
        try {
            ...
            const imageBuffer = fs.readFileSync(path.join(basePath, fileDirectory, file));
            ...
            for (let indexWidth = 0; indexWidth < widths.length; indexWidth++) {
                ...
                const basePathToStoreOptimizedImages = isStaticImage ||
                    ...
                    ? "public"
                    : basePath;
                const optimizedFileNameAndPath = path.join(basePathToStoreOptimizedImages, fileDirectory, exportFolderName, `${filename}-opt-${width}.${extension.toUpperCase()}`);
                ...
                // the optimization.
                if (optimizedOriginalWidthImagePath &&
    ...
    for (let index = 0; index < allGeneratedImages.length; index++) {
        ...
        const fileInBuildFolder = path.join(exportFolderPath, filePath.split("public").pop());
    ...
    const optimizedImagesFolders = findSubfolders(imageFolderPath, exportFolderName);
    ...
    if (unusedImages.length > 0)
        console.log(`Deleted ${unusedImages.length} unused image${unusedImages.length > 1 ? "s" : ""} from the optimized images folders.`);
